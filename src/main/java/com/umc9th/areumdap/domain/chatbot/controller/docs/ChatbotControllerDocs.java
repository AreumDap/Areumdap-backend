...
public interface ChatbotControllerDocs {
    ...
    @Operation(summary = "오늘의 추천 질문 배정", description = "당일 배정된 질문이 5개 미만이면 새로 배정합니다. 질문 데이터는 반환하지 않습니다.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "추천 질문 배정 성공"),
    ...
    ResponseEntity<ApiResponse<Void>> getChatbotRecommend(
    ...
    ResponseEntity<ApiResponse<GetChatbotRecommendsResponse>> getAssignedQuestions(
            @AuthenticationPrincipal Long userId
    );
